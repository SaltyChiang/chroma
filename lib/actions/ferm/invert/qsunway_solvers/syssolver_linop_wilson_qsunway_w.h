...
#ifndef __syssolver_linop_qsunway_wilson_h__
#define __syssolver_linop_qsunway_wilson_h__
...
namespace Chroma
{
  ...
  //! Solve a Wilson Fermion System using the QSUNWAY inverter
  /*! \ingroup invert
 *** WARNING THIS SOLVER WORKS FOR Wilson FERMIONS ONLY ***
   */

  class LinOpSysSolverQSUNWAYWilson : public LinOpSystemSolver<LatticeFermion>
  {
  public:
     ...
     */
    LinOpSysSolverQSUNWAYWilson(Handle<LinearOperator<T>> A_,
                                Handle<FermState<T, Q, Q>> state_,
                                const SysSolverQSUNWAYWilsonParams &invParam_) : A(A_), invParam(invParam_)
    {
      QDPIO::cout << std::endl;
      QDPIO::cout << "LinOpSysSolverQSUNWAYWilson:" << std::endl;
      ...
      // Dslash type
      qsunway_inv_param.dslash_type = QSUNWAY_WILSON_DSLASH;
      ...
      qsunway_inv_param.kappa = 1.0 / (2 * toDouble(massParam));
      // qsunway_inv_param.clover_coeff = 0.0; // Always true for Wilson
      ...
      // qsunway_inv_param.mass_normalization = QSUNWAY_ASYMMETRIC_MASS_NORMALIZATION;
      ...
      // qsunway_inv_param.preserve_source = QSUNWAY_PRESERVE_SOURCE_YES;
      // qsunway_inv_param.use_init_guess = QSUNWAY_USE_INIT_GUESS_NO;
      ...
      QDPIO::cout << "Init finished." << std::endl;
    }

    //! Destructor is automatic
    ~LinOpSysSolverQSUNWAYWilson()
    {
    ...
    SystemSolverResults_t operator()(T &psi, const T &chi) const
    {
      ...
      QDPIO::cout << "QSUNWAY_" << solver_string << "_WILSON_SOLVER: " << res.n_count << " iterations. Rsd = " << res.resid << " Relative Rsd = " << rel_resid << std::endl;
  ...
  private:
    ...
    const SysSolverQSUNWAYWilsonParams invParam;
    Real invMassParam;
